...
<img src="https://img.shields.io/github/actions/workflow/status/quale-lang/quale/ci.yml?branch=main&label=Tests" />
</div>
</p>
...
Installing quale is simple if you have a working rust tool chain. The compiler
is written in Rust. You can fetch the source and directly build it yourself.
...
optimization options aren't available yet. They are placeholders for now.
...
will assume 0 to represent heads and 1 to represent tails. But the fairness of
...
and `qbit` refers to the type in quale.

We will work on this problem stepwise. Firstly, the `main` function which
usually is the entry point in many languages is back. We also start with the
...
entry point for execution in our resulting code.
...
return classical value, but at least one of its parameters should be a quantum
...
certain things we aren't allowed to do in a non-deterministic function, like
...
which is Peter Selinger's type system presented in his paper [[1]](https://www.mathstat.dal.ca/~selinger/papers/papers/qlambdabook-2up.pdf), handles
